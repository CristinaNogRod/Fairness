...
def run_experiment(
    dataset_path='datasets/proc/crafted_obesity.csv',
    report_path='outputs/report_obesity.json'
):
    ...
    model = OutlierDetector(alpha=0.5, gamma=0.2)
    _, _ = model.fit(X_train, pv_train, batch_size=512, epochs=100, val_X=None, val_pv=None)
    ...
    _, _ = model.fit(X_train, pv_train, batch_size=1024, epochs=100, verbose=False, early_stop=True)
