...
def run_optim(X_train, y_train, pv_train, bayesian_runs=32, seed=33):
    opt = BayesSearchCV(
        ...
        random_state=seed,
        fit_params= dict(
              batch_size=512, epochs=5, val_X=None, val_pv=None
        ),
        verbose=1,
        cv=2
    )
...
def run_eval(X_train, pv_train, X_test, pv_test, y_test, hyperparams, epochs=6):
    ...
    model.fit(X_train, pv_train, batch_size=512, epochs=epochs, val_X=None, val_pv=None)
